...
Jangan gunakan simbol, emoji, atau format lain selain teks biasa.`
        ...
        img.onload = async () => {
          ...
          if (ctx) {
            ...
            try {
              ...
              if (!analysisResponse.ok) throw new Error(analysisData.error || "Gagal menganalisis gambar")
              const analysisText =
  ...
  const speak = async () => {
    ...
    try {
      ...
      if (!ttsUrl) throw new Error("NEXT_PUBLIC_TTS_URL belum dikonfigurasi")
      ...
      setIsPlaying(false)
      setOutput("Gagal memutar audio. Periksa konfigurasi dan koneksi server TTS.")
      cleanupAudio()
    }
  }
